...
test("postSegmentLog rejects warmup program exercises without writing logs", async () => {
  ...
  assert.equal(res.statusCode, 400);
  assert.equal(res.body?.error, "warmup/cooldown segments are not loggable");
...
test("postSegmentLog rejects cooldown program exercises without writing logs", async () => {
  const queries = [];
  const handlers = createSegmentLogHandlers({
    async connect() {
      return {
        async query(sql, params) {
          ...
          if (sql.includes("SELECT pe.id AS program_exercise_id")) {
            return {
              rows: [{ program_exercise_id: VALID_UUID, segment_type: "cooldown", strength_primary_region: null }],
              rowCount: 1,
            };
          }
          throw new Error(`Unexpected SQL: ${sql}`);
