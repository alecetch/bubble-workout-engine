...
adminRepRulesRouter.patch("/rep-rules/rules/:rule_id", async (req, res) => {
  try {
    ...
    for (const key of PRESCRIPTION_FIELDS) {
    ...
    await auditLog(req, {
      ...
      detail: Object.fromEntries(
        [...PRESCRIPTION_FIELDS].filter((k) => k in body).map((k) => [k, body[k]]),
      ),
    });
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [
        ...
        toNullableText(body.movement_pattern),
        toNullableText(body.swap_group_id_2),
        toNullableText(body.equipment_slug),
        repLow,
        repHigh,
        repsUnit,
        toInt(body.rir_min, 0),
        toInt(body.rir_max, 0),
        toInt(body.rir_target, 0),
