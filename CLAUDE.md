...
5. **Review and update the reference docs** to reflect what was actually built. Read the merged code and compare it against the three reference docs in `docs/reference/`. Make any changes needed:
   ...
   - **`docs/reference/architecture.md`** — three sections require a **mandatory check on every merge**, regardless of what the feature touched: (a) the test-count line — increment it if any tests were added or removed; (b) the Flyway migration table — add a row for every new V-migration; (c) any "known gaps" or "status" notes — remove or update entries that are no longer accurate. Beyond those, update the ASCII diagram and route table if the feature added new services, changed the pipeline steps, introduced new middleware, changed the high-level data flow, or added new internal components.
