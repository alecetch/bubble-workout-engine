CREATE TABLE warmup_exercise (
  warmup_exercise_id      text PRIMARY KEY,
  name                    text NOT NULL,
...
CREATE INDEX idx_warmup_exercise_regions_gin ON warmup_exercise USING gin (target_regions_json);
CREATE INDEX idx_warmup_exercise_equipment_gin ON warmup_exercise USING gin (equipment_items_slugs);

CREATE TABLE warmup_exercise_media (
  warmup_exercise_id           text PRIMARY KEY REFERENCES warmup_exercise(warmup_exercise_id) ON DELETE CASCADE,
