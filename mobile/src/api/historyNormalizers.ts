...
export function normalizeOverview(raw: unknown): HistoryOverviewResponse {
...
export function normalizePrograms(raw: unknown): HistoryProgramItem[] {
...
export function normalizePersonalRecords(raw: unknown): HistoryPersonalRecordItem[] {
  return asArray(raw).map((item) => {
    const row = asObject(item);
    return {
...
export function normalizeExerciseSearch(raw: unknown): ExerciseSearchItem[] {
  return asArray(raw)
    .map((item) => {
      const row = asObject(item);
      return {
...
export function normalizeExerciseHistory(raw: unknown): ExerciseHistoryResponse {
  const root = asObject(raw);
...
export function normalizeHeaviestLift(raw: unknown): HeaviestLift | null {
  if (raw == null) return null;
  const row = asObject(raw);
  return {
    exerciseId: asString(row.exerciseId),
...
export function normalizePrsFeed(raw: unknown): PrsFeedResponse {
  const root = asObject(raw);
  const modeRaw = asString(root.mode);
  const mode: PrsFeedResponse["mode"] =
    modeRaw === "prs_28d" || modeRaw === "prs_90d" || modeRaw === "heaviest_28d"
      ? modeRaw
      : "heaviest_28d";
...
export function normalizeLoggedExercises(raw: unknown): LoggedExerciseItem[] {
  const root = asObject(raw);
...
export function normalizeExerciseSummaryLift(raw: unknown): ExerciseSummaryLift | null {
  if (raw == null) return null;
  const row = asObject(raw);
  return {
    weightKg: asNumber(row.weightKg, 0),
...
export function normalizeExerciseSummary(raw: unknown): ExerciseSummaryResponse {
  const root = asObject(raw);
  return {
