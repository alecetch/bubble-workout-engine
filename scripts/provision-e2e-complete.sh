...
auth_body="{\"email\":$(json_escape "$E2E_COMPLETE_EMAIL"),\"password\":$(json_escape "$E2E_COMPLETE_PASSWORD")}"
auth_response="$tmp_dir/auth.json"

echo "Provisioning E2E complete account ${E2E_COMPLETE_EMAIL}"
...
if [[ -z "$program_id" ]]; then
  echo "Generating E2E Final Block"
...
echo "Trimming program ${program_id} to one scheduled day"
psql -v ON_ERROR_STOP=1 <<SQL
WITH keep AS (
...
SET scheduled_date    = CURRENT_DATE,
    scheduled_weekday = to_char(CURRENT_DATE, 'Dy'),
    week_number       = 1
WHERE program_id = '${program_id}';

UPDATE program
SET program_title = 'Final Block',
    status        = 'active',
    is_primary    = true
WHERE id = '${program_id}';
SQL

echo "E2E complete account ready: user_id=${user_id} program_id=${program_id}"
